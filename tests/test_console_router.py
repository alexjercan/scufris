...
Every route here is console-only, so each is asserted on what it translates:
which collaborator owns the rule, and which status its refusal becomes. The
four compatibility aliases that used to live beside them (`usage`, `memory`,
`account`, `health`) are gone, and `test_the_alias_routes_are_gone` is the guard
that they do not come back - `/api/agents/orchestrator/*` is the one surface for
those answers now.
"""
...
from scufris.api.console import (
    AgentConfigUpdate,
    ConsoleDeps,
...
from scufris_core import Database, EventBus
...
class FakeRunService:
    ...
    record the info and config views resolve through, and the forked turn."""
...
@pytest.fixture
def console_rig(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> ConsoleRig:
    rig = ConsoleRig(_settings(tmp_path))
    ...
    monkeypatch.setattr("scufris.api.console.get_backend", lambda _name: rig.backend)
    return rig


@pytest.fixture
def console_client(console_rig: ConsoleRig) -> Iterator[TestClient]:
    with TestClient(console_rig.app) as client:
        yield client
...
def test_the_alias_routes_are_gone(console_client: TestClient, path: str) -> None:
    """The four singular routes that duplicated `/api/agents/orchestrator/*` are
    deleted, not re-added: one surface per question, so the two cannot drift into
    two different answers."""
    assert console_client.get(path).status_code == 404, path


def test_agent_info_reads_the_record_not_the_codex_model_slot(
    console_rig: ConsoleRig, console_client: TestClient
) -> None:
    ...
    console_rig.runs.known[ORCHESTRATOR_ID] = _orchestrator(
        backend="claude", model="opus"
    )
    body = console_client.get("/api/agent/info").json()
    assert body["model"] == "opus"
    assert body["enabled"] is True
    assert console_rig.runs.calls == [("require_agent", ORCHESTRATOR_ID)]
...
def test_the_config_view_layers_live_settings_over_the_record(
    ...
    console_rig.store.writable = False
    body = console_client.get("/api/agent/config").json()
...
def test_a_config_patch_is_the_stores_whitelist_translated(
    ...
    response = console_client.patch(
        "/api/agent/config", json={"agent_tools_enabled": False}
    )
    assert response.status_code == 200, response.text
    assert console_rig.store.applied == [{"agent_tools_enabled": False}]
...
def test_the_config_patch_maps_every_store_refusal(
    console_rig: ConsoleRig,
    console_client: TestClient,
    raised: Exception,
    status: int,
) -> None:
    """The store decides; the router only says it in HTTP."""
    console_rig.store.raises = raised
    response = console_client.patch("/api/agent/config", json={"poll_seconds": 2.0})
...
def test_the_tool_runner_refuses_a_disabled_tool_before_it_looks_for_it(
    ...
    rig = ConsoleRig(_settings(tmp_path, disabled_tools=["host_reboot"]))
...
def test_the_tool_runner_is_404_for_a_tool_no_server_owns(
    ...
    response = console_client.post("/api/agent/tools/no_such_tool/run", json={})
...
def test_the_session_list_is_the_registrys_and_is_newest_first(
    ...
    monkeypatch.setattr(
        "scufris.api.console.session_info",
        lambda _backend, _settings, sid: infos.get(sid),
    )
    body = console_client.get("/api/agent/sessions").json()
    assert [s["id"] for s in body["sessions"]] == ["sess-new", "sess-old"]
    assert body["current"] == "sess-old"
    assert ("orchestrator_sessions", None) in console_rig.agents.calls
...
def test_switching_sessions_serializes_on_the_orchestrator_key(
    ...
    assert console_rig.supervisor.keys == [ORCHESTRATOR_ID]


def test_switching_without_a_session_id_is_422(console_client: TestClient) -> None:
    assert (
        console_client.post("/api/agent/session", json={"action": "switch"}).status_code
        == 422
    )
...
def test_deleting_a_session_removes_it_provider_side_and_forgets_it(
    ...
    response = console_client.delete("/api/agent/session/sess-old")
    assert response.json() == {"deleted": True, "current": None}
    assert ("delete_session", "sess-old") in console_rig.backend.calls
    assert ("forget_orchestrator_session", "sess-old") in console_rig.agents.calls
    assert console_rig.supervisor.keys == [ORCHESTRATOR_ID]
...
def test_a_backend_with_no_provider_delete_still_drops_the_session(
    ...
    console_rig.backend.deleted = False
    assert (
        console_client.delete("/api/agent/session/sess-old").json()["deleted"] is False
    )
    assert ("forget_orchestrator_session", "sess-old") in console_rig.agents.calls
...
def test_forking_clears_the_session_before_it_launches(
    ...
    kinds = [name for name, _ in console_rig.runs.calls]
    assert kinds == ["fork_seed", "launch", "drain"]
    # Launched off a record whose session was already cleared, not off a re-read.
    assert console_rig.runs.launched == [(ORCHESTRATOR_ID, None, "the fork prompt")]
    assert ("set_orchestrator_session", None) in console_rig.agents.calls
    ...
    assert console_rig.supervisor.keys == []
...
def test_a_disabled_agent_refuses_every_mutating_session_route(
    ...
    rig = ConsoleRig(_settings(tmp_path, agent_enabled=False))
    with TestClient(rig.app) as client:
...
def _forbidden(*args: Any, **kwargs: Any) -> Any:
    raise AssertionError(
        "the console router constructed a store, a database or a settings "
        "object; the whole point of the deps dataclass is that it does not"
    )


def test_the_console_router_reaches_for_nothing(
    console_rig: ConsoleRig,
    console_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    for path in (
        ...
        assert console_client.get(path).status_code == 200, path
