...
def test_throttling_sums_cores_and_takes_the_package_maximum(tmp_path: Path) -> None:
    """Package counters are per-package, so summing them multiplies by core count.

    Two CPUs of one package each report the SAME 78 package events (measured on
    the real host); the correct total is 78, not 156.
    """
    for cpu in range(2):
    ...
    assert counters.core_events == 10  # summed across cores
    assert counters.package_events == 78  # NOT 156
