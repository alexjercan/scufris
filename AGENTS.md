...
| `packages/chat/src/scufris_chat/README.md` | The conversation, event, delivery and provider-session tables, the `event_seq` invariant, the typed actor, the per-channel delivery guarantee, the bounded attributed context assembly, the connection-passing rule |
