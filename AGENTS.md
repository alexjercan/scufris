...
| `packages/*/pyproject.toml` | Workspace members: one distribution each, own dependency list. `packages/core` -> `scufris_core`, the engine, `Database`, `Base`, `logsetup`, `eventbus` (`EventBus`) and `supervisor` (`Supervisor`, `RunState`, `RunPhase`) - the allowlist `test_core_is_domain_free` enforces; `packages/host` -> `scufris_host`, read-only inspection, which depends on no sibling; `packages/hostd` -> `scufris_hostd`, the root helper and the `scufris-hostd` console script, which depends on `core` and `host` and is pinned exactly by the root; `packages/hostctl` -> `scufris_hostctl`, the unprivileged client that drives the helper, which depends on `core`, `host` and `hostd`; `packages/chat` -> `scufris_chat`, the conversation, its events, their per-channel delivery, the provider-session cache assembled context re-seeds and the operator decision only a committed operator event mints, which depends on `core` alone |
...
| `packages/chat/src/scufris_chat/README.md` | The conversation, event, delivery and provider-session tables, the `event_seq` invariant, the typed actor, the operator decision a stop gate takes, the per-channel delivery guarantee, the bounded attributed context assembly, the connection-passing rule |
...
- Nix CLI: route new CLI calls through `scufris_host`'s `nix_cli`.
