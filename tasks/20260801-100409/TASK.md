...
- [ ] Write the five proofs first and watch each fail on the base commit:
...
- [ ] Declare the schema in `scufris/db/models.py` and autogenerate ONE Alembic
...
- [ ] Add the loop-thread guard to `scufris/db/engine.py` (DECISION.md 1):
...
- [ ] Sweep the loop-thread callers this makes illegal. The 20 known sites are
...
- [ ] Promote the one-handle-per-process memo out of `scufris/mcp_stores.py`
...
- [ ] Rewrite `scufris/agent_store/registry.py` and `outcomes.py` as
...
- [ ] Rewrite `scufris/agent_store/store.py` onto the core: delete `_agents`,
      ...
      route.
- [ ] Split `agent_store/store.py` while rewriting it: it is 595 lines against
...
- [ ] Close the read-modify-write windows INSIDE the transaction, not around
      ...
      read-modify-write.
- [ ] Commit the whole completion path in ONE transaction: the agent row's
      ...
      it names.
- [ ] Rewrite `scufris/settings_store.py` onto `settings_override` rows,
...
- [ ] Rewrite `scufris/reasoning_store.py` onto `reasoning_turn` rows: an
...
- [ ] Add the legacy importers to `scufris/db/legacy.py` on the existing
...
- [ ] Give the reasoning files an explicit gate key rather than `source.name`:
...
- [ ] Wire the new importers into `scufris.db.open_state_database` next to
      `import_projects`, so they run after the migration and before any store
      read.
- [ ] Add duplicate-import and corrupt-input tests per source in
...
- [ ] Update the ~75 store constructions across `tests/test_agent_store.py`,
...
- [ ] Update `scufris/README.md` section 9 (agents, sessions, outcomes,
