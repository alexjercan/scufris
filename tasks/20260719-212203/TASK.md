...
- [ ] `scufris/sessions.py`: models `RateWindow` (used_percent, window_minutes,
...
- [ ] `scufris/sessions.py`: `list_sessions(codex_home, cwd) -> list[SessionInfo]`
      ...
      bounded reads.
- [ ] `scufris/agent.py`: generalize `CodexCliAgent._thread_id` to a current
...
- [ ] Tests: `tests/test_sessions.py` (write fake rollout `.jsonl` into a tmp
...
- [ ] `nix develop` full check green (ruff, ruff format, mypy, pytest) and a live
