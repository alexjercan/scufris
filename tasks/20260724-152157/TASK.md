...
- [ ] Verify-first (rollout timing): inspect a real `~/.codex/.../rollout-*.jsonl`
...
- [ ] Add a `StreamSessionStarted` StreamEvent (`kind="session_started"`,
      `session_id: str`) to the union in `scufris/agent.py:63-94`.
- [ ] Emit it in `_stream_app_server` right after `new_thread_id` is set
      (`agent.py:573-575`), before `turn/start`, whenever `new_thread_id` is truthy
      (covers both `thread/start` fresh and `thread/resume`).
- [ ] In `_launch_agent_turn`'s `turn_stream` (`scufris/app.py:1177-1195`), on a
...
- [ ] Confirm idempotency: `set_current` sets the current pointer AND appends to
      ...
      entry.
- [ ] Confirm `/api/agent/sessions` (`scufris/app.py:~1710-1731`) surfaces the
...
- [ ] Frontend type: add the `StreamSessionStarted` shape to `web/src/common.ts`
...
- [ ] Backend test: drive a codex-shaped stream that yields `StreamSessionStarted`
...
- [ ] Failed-turn test: a stream that yields `StreamSessionStarted` then
...
- [ ] Write DECISION.md for the early-session-event mechanism; index it in the
      umbrella GOAL.md Decisions section.
- [ ] Run the full gate: `nix flake check` and `cd web && npm run ci`; both green.
