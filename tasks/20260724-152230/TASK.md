...
- [ ] Confirm the orchestrator run is reachable via `/api/agents/orchestrator/status`
...
- [ ] On the landing (`web/src/agent-view.ts`), when `/api/agent/sessions` returns
...
- [ ] Add a `reattach` to the landing's `createAgentChat` config that, gated on the
...
- [ ] Ensure the landing's `onSessionStarted` (from task 1's handler) updates
      `currentSessionId` live, so a turn STARTED in this tab also pins the new
      session id for forks/switcher without a refresh.
- [ ] Frontend tests (`web/src/agent-view.test.ts` or the shared harness): (a) on
