...
- [x] `scufris/hostd/actions.py`: the typed action taxonomy. `ActionKind`
...
- [x] `scufris/hostd/preview.py`: preview per risk class, honest where there is
...
- [x] `scufris/hostd/actions.py` (R2 floor): `gc_older_than` resolves the
      generation list FIRST and NAMES the generations it will delete in the argv
      (`nix-env --profile /nix/var/nix/profiles/system --delete-generations N...`),
      so the floor is a property of the command rather than of the preview beside
      it. `--delete-older-than` is not used at all: it keeps only the CURRENT
      generation and is otherwise purely age-based, so it cannot express the
      floor. Never a bare `nix-collect-garbage -d`. The first implementation
      displayed the floor and emitted the flag anyway - review round 1, R1.4.
- [x] `scufris/hostd/engine.py`: the proposal registry lives IN THE HELPER.
...
- [x] `scufris/hostd/audit.py`: append-only single-line JSON. Records
...
- [x] `scufris/hostd/protocol.py` + `server.py`: newline-delimited JSON frames
...
- [x] `scufris/hostd/main.py` and the `scufris-hostd` console script. Fail
      closed: no secret configured means the helper refuses to start.
- [x] The reversal path. R1 records prior `ActiveState`/`SubState` and offers
      restoration of it, acknowledging that a restarted daemon lost in-memory
      state. R2 records that it CANNOT be undone, and the rendered preview says
      so in the words the operator reads (`NO UNDO:`). This step originally also
      asked R2 to take a STRONGER CONFIRMATION than the reversible classes;
      nothing here varies the approval by risk class, and it was ticked anyway -
      CORRECTED in review round 2, R2.5. A differentiated confirmation is an
      interface act and this task ships no approval UI, so it is handed to
      20260729-125040, whose TASK.md now carries it as an explicit requirement.
      What IS delivered here is the substrate that makes it possible: `risk` and
      `reversal.possible` ride every `ProposalView`.

### B. One execution path

- [x] Make `Supervisor` and `EventBus` generic in their event type
      ...
      claiming green.
- [x] `scufris/hostclient.py`: the app-side async client for the socket. Apply
...
- [x] `auth.py` + the `app.py` middleware: an `OPERATOR_ONLY_PATHS` set consulted
...
- [x] Endpoints: `POST /api/host/actions` (propose - a session OR the machine
...
- [x] An MCP tool `propose_host_action` in `mcp_server.py` so an agent can
      propose and read its own proposal, and has no tool that can approve one.
- [x] Config in `scufris/config.py`: `hostd_socket` and `hostd_secret`. With
      hostd absent, every mutating endpoint answers "no privileged helper
      configured" rather than half-working. NO separate `hostd_enabled` knob was
      added (this step originally asked for one): an empty secret already means
      "not configured", and a second flag that could disagree with it is a state
      to get wrong rather than a feature. CORRECTED in review round 1, R1.15.
      Also corrected there: the secret CANNOT be kept out of `os.environ` the way
      `auth_api_token` is, because it arrives through an EnvironmentFile - it is
      stripped from every subprocess environment instead (`SECRET_ENV_VARS`).

### D. Packaging and deployment

- [x] `[project.scripts] scufris-hostd` in `pyproject.toml`.
- [x] `nix/scufris-hostd.nix`: a NixOS SYSTEM module exporting
      ...
      diffable act.
- [x] Extend `nix/tests/scufris-vm.nix` (or add a sibling) to boot the helper and
...
- [x] `tests/test_host_actions.py`: the DoD tests plus the adversarial set -
...
- [x] `tests/test_hostd_audit.py`: rotation on size, oldest-first pruning, and
      that no code path deletes or rewrites an entry.
- [x] `examples/host_action.py`: propose -> preview -> approve -> apply -> audit
      -> revert end to end against a `FakeRunner`, printing what an operator
      would see.
- [x] Docs in the SAME task: the AGENTS.md deployment section (the second sops
