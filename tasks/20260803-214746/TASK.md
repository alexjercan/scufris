...
- [ ] Write `tests/test_package_boundaries.py` FIRST, with a helper
      ...
      `ModuleNotFoundError`).
- [ ] Root `pyproject.toml`: add `[tool.uv.workspace] members = ["packages/*"]`
...
- [ ] Widen `testpaths` at `pyproject.toml:43` to
...
- [ ] Create `packages/core/pyproject.toml`: name `scufris-core`,
...
- [ ] `git mv scufris/db/engine.py packages/core/src/scufris_core/engine.py`.
...
- [ ] Move the `Base` class (`scufris/db/models.py:42`) to
      ...
      need NO edit.
- [ ] `git mv scufris/logsetup.py
      packages/core/src/scufris_core/logsetup.py` (see the logsetup decision in
      the Notes). Re-point its twelve importers under `scufris/` and
      `tests/test_logsetup.py` at `scufris_core`; update the `logsetup` mention
      in `scufris/README.md`.
- [ ] Write `packages/core/src/scufris_core/__init__.py` as the ONLY surface a
...
- [ ] Re-point the direct importers of the moved engine. The census is FOUR
...
- [ ] Leave `scufris/db/migrate.py` at the root. It is Alembic-coupled and
...
- [ ] `scripts/check_file_size.py`: add `"packages"` to `COVERED_ROOTS`.
...
- [ ] Leave `flake.nix:74` `members` commented. It is an EDITABILITY filter on
      ...
      regenerated `uv.lock`.
- [ ] Re-enter `nix develop` (or run `nix flake check`) after `uv lock`. The dev
...
- [ ] Add `examples/core_unit_of_work.py`: open a temp SQLite database through
...
- [ ] Add `tests/test_examples.py` running an explicit OPT-IN list of offline
...
- [ ] Write `tasks/20260803-213242/DECISION.md` - the EPIC folder, per this
...
- [ ] Update `scufris/README.md` section 9 and the `AGENTS.md` sources table:
