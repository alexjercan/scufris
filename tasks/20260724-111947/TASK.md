...
- [ ] Grow the `SessionRegistry` entry shape (`agent_store.py`) from
      ...
      do not use it here.
- [ ] Add registry methods: `sessions_for(agent_id, backend) -> list[str]` (the
...
- [ ] Repoint `AgentStore` call sites: `mark_finished` uses `add` (was `set`);
...
- [ ] Rewrite `GET /api/agent/sessions` (`app.py`) to build the list from
      ...
      new `_session_info(backend, settings, sid) -> SessionInfo | None` helper
      (title = first `user` message from `backend.read_transcript`, `started_at`
      = that message's `ts`, `updated_at` = `backend.read_status(...).updated_at`
      as a UTC datetime; `git_branch`/`cwd` left None), drop unhydratable ids,
      sort newest-first by `updated_at`. `current` stays
      `agents.orchestrator_session_id()`. Remove the `list_sessions` import from
      `app.py`.
- [ ] Make `DELETE /api/agent/session/{session_id}` also call
...
- [ ] Tests (registry, `tests/test_agent_store.py`): `add` accumulates multiple
      ...
      history.
- [ ] Test (the leak regression, `tests/test_app.py`): write two codex rollouts
...
- [ ] Test (multi-session): two orchestrator sessions both appear, newest first;
...
- [ ] Append a NOTES.md design/fix record (what changed, the leak diagnosis, the
