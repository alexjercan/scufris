...
- [ ] Add `scufris/projects.py`: a `Project` pydantic model
...
- [ ] Store methods: `list()`, `get(id)`, `create(name, cwd, language,
      ...
      the config store.
- [ ] Add CRUD endpoints in `scufris/app.py`: `GET /api/projects` (list),
      ...
      `SettingsStore`.
- [ ] Tests: store round-trip (create -> fresh store over same dir sees it;
...
- [ ] Update `.env.example` only if a new knob is added (none expected -
