...
- [ ] Add `scufris/backends.py`: `BackendStatus` (pydantic:
...
- [ ] `CodexBackend(mode)` (mode `"exec"` | `"app_server"`, `name = mode`):
...
- [ ] `MockBackend` (`name="mock"`): `stream` yields canned events (reuse the
...
- [ ] Add `sessions.rollout_mtime(codex_home, session_id) -> float | None`
      (public, reuses `_find_rollout`) for `updated_at`.
- [ ] Tests `tests/test_backends.py`: CodexBackend.stream over the fake-codex
...
- [ ] PROBE (best-effort, per the user): a small script/NOTES that attempts one
