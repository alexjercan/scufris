...
- MINOR: `psutil.net_io_counters()` can theoretically return `None` on a host with
...
- MINOR: the smoke test's `mem.used >= 0` assertion is trivially true; the
  surrounding assertions (total > 0, cpu in range, tz-aware timestamp) carry the
  real signal, so coverage is adequate.
- NIT: first sample right after construction has a tiny delta window so CPU can
...
The collector meets its Definition of Done, sits behind a fakeable
seam, and the checks (ruff, ruff format, mypy, pytest) are green. The two MINOR
