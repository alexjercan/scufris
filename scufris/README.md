...
| `packages/chat` -> `scufris_chat` | the conversation Scufris owns: the `conversation`, `event`, `delivery` and `provider_session` tables, the per-conversation transcript position, the typed `Actor`, per-channel delivery idempotency keyed off the event itself, the provider session held as a CACHE that bounded attributed assembly re-seeds, and a store of ten functions over a connection the CALLER opened. Depends on `core` alone ([README](../packages/chat/src/scufris_chat/README.md)) |
...
python -m pytest tests/test_db_migrations.py tests/test_db_schema.py
```
