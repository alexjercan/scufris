...
| `packages/chat` -> `scufris_chat` | the conversation Scufris owns: the `conversation`, `event` and `delivery` tables, the per-conversation transcript position, the typed `Actor`, per-channel delivery idempotency keyed off the event itself, and a store of seven functions over a connection the CALLER opened. Depends on `core` alone ([README](../packages/chat/src/scufris_chat/README.md)) |
